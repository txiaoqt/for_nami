...
The DFD covers the implemented scope of the site, including account access, policy agreement, public information browsing, program/event registration, youth service request handling, administrative management, and audit-supported governance.
...
```mermaid
flowchart LR
    E1[/"E1 Youth User"/] -->|Sign up/login, browse, register, submit request| P0((0.0 LYDO Connect System))
    E2[/"E2 Admin / Staff"/] -->|Manage content, records, users, roles, requests, reports| P0
    E3[/"E3 External Registration Source"/] -->|Registration rows for sync| P0

    P0 -->|Public info, registration status, request updates, policy prompts| E1
...
1. Youth users create accounts, log in, and access protected features through Process `1.0`, which validates identity using `D1` and access rules from `D2`.
...
5. Youth service requests and follow-ups are handled by Process `5.0`, which creates and updates request records in `D8`.
...
The DFD set (Context, Level 1, and Level 2) demonstrates that LYDO Connect supports end-to-end data handling for youth users and admin/staff personnel: from access and consent, to registration and service operations, up to governance reporting and audit traceability.
